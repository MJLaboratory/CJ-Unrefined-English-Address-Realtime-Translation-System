...
#os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'C:\Dev\cj_challenge2\\2023CJ_DaehanTongun_challenge\김민지\googleTransAPI\\brilliant-scene-392701-53df09bf1204.json'
...
def extract(txt):
    ...
    match = ('-daero', 'daero', 'ro', '-ro', 'gil', '-gil', 'gu', '-gu', '-si', 'si','underground', 'beon-gil',  
             '로', '길', '구' , '번길')
    eles = []
    for ele in mylist:
...
# Define a function to translate text using the translator
def translate_text(text):
    translated = translator.translate(text, src="en", tgt="kr")
    return translated

# 번역 후 붙은 '번지'와 '층' 제거
def remove_bunji(str):

    str = str.replace('번지', '')
    result = str.replace('층', '')
    #print(result)
    return result
...
def add_comma(text):
    ...
    return text
#df['comma_added'] = df.apply(lambda x: add_comma(x['1']), axis=1)
#df.head(10)
...
def get_translated_address(text):
    ...
    if text7=='답 없음':
        ...
        text7 = find_address(translate_text(new_text))
        if text7 == '답 없음':
